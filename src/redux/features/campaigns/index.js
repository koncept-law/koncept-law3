...
export const SelectedDocumentTemplateThunkMiddleware = ({ documents },
  callback
) => {
  return async (dispatch) => {
    try {
      dispatch(setLoader({ loader: true }));
      dispatch(setCampaigns({ selectedDocs: documents }));
    } catch (error) {
      // console.log(error);
      if (error.response?.data) {
        toastify({ msg: error.response.data.message, type: "error" });
...
export const sendDocumentVaribleValuesThunkMiddleware = ({
  ...
  return async (dispatch) => {
    try {
      ...
      // const response = await axios.post("https://p3.kcptl.in/docs/mergeMapped", {
...
export const downloadCampaignSinglePdfFileThunkMiddleware = ({ campaignName }) => {
  return async (dispatch) => {
    try {
      ...
      // link.target = "_blank"
      // const link = document.createElement("a");
      // link.href = `https://p3.kcptl.in/docs/downloadpdf?campaignName=${campaignName}`
      // link.target = "_blank";
      // document.body.appendChild(link);
      // link.click();
...
export const downloadDocumentCategorySinglePdfThunkMiddleware = ({ campaignName,
  ...
  return async (dispatch) => {
    try {
      ...
      // method 1
      const response = await axios.post(`/docs/DownloadSingleMainPdf`,
        {
          campaignName: campaignName,
          templateId: templateId,
          links: links,
      ...
      // console.log("response status of download single pdf by category", response.status)
